...
print(f"\n  [OK] Saved -> outputs/tables/fold_metrics_fullness.csv")
...
plt.close(fig)
print("  [OK] Saved -> outputs/figures/roc_curves_fullness.png")
...
plt.close(fig)
print("  [OK] Saved -> outputs/figures/pr_curves_fullness.png")

# ── 5c. ROC curves — Task B (per cup class, OvR) ─────────────────────────
...
for ci, cname in enumerate(class_names):
    ...
    ax.set_title(f"OvR ROC — {cname}", fontweight="bold")

    for model_name, color in MODEL_COLORS.items():
        data = roc_data_b[model_name][cname]
        if not data["fprs"]:
            continue
        interp_tprs = []
        for fpr, tpr in zip(data["fprs"], data["tprs"]):
        ...
        ax.fill_between(mean_fpr,
                         mean_tpr - np.std(interp_tprs, axis=0),
                         mean_tpr + np.std(interp_tprs, axis=0),
...
fig.suptitle("One-vs-Rest ROC Curves — Cup Type Classification\nGroupKFold (K=5) Mean +/- Std",
             fontsize=13, fontweight="bold", y=1.01)
fig.tight_layout()
fig.savefig(OUT_FIGS / "roc_curves_cup_type.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  [OK] Saved -> outputs/figures/roc_curves_cup_type.png")

# ── 5d. Feature importance (all 3 models, Task A) ─────────────────────────
fig, axes = plt.subplots(1, 3, figsize=(17, 6))

for ax, (model_name, color) in zip(axes, MODEL_COLORS.items()):
    imp  = feat_imp_a[model_name]
    # Normalise to [0, 1]
    imp_norm = imp / imp.max() if imp.max() > 0 else imp
    order = np.argsort(imp_norm)[::-1]
    top_n = 15
    feat_names_top = [FEATURE_COLS[i] for i in order[:top_n]]
    imp_top = imp_norm[order[:top_n]]

    bars = ax.barh(feat_names_top[::-1], imp_top[::-1],
                   color=color, edgecolor="white", linewidth=0.8)
    ax.set_title(f"{model_name}\n(Is_Full task)", fontweight="bold")
    ax.set_xlabel("Normalised Importance")
    ax.set_xlim([0, 1.12])
    for bar, val in zip(bars[::-1], imp_top[::-1]):
...
plt.close(fig)
print("  [OK] Saved -> outputs/figures/feature_importance.png")
...
plt.close(fig)
print("  [OK] Saved -> outputs/figures/metrics_comparison_fullness.png")

# ── 5f. CV metrics bar chart — Task B ────────────────────────────────────
metrics_b_plot = ["Accuracy", "F1-Macro", "F1-Weighted", "AUC-ROC-Macro"]
x2 = np.arange(len(metrics_b_plot))

fig, ax = plt.subplots(figsize=(10, 6))
for i, (model_name, color) in enumerate(MODEL_COLORS.items()):
    row = summary_b[summary_b["Model"] == model_name].iloc[0]
    means = [row[f"{m}_mean"] for m in metrics_b_plot]
    stds  = [row[f"{m}_std"]  for m in metrics_b_plot]
    offset = (i - 1) * width
    bars = ax.bar(x2 + offset, means, width, label=model_name,
                  color=color, yerr=stds, capsize=4,
                  edgecolor="white", linewidth=1.0, alpha=0.9)
    for bar, val in zip(bars, means):
        ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.008,
...
ax.set_xticks(x2)
ax.set_xticklabels(metrics_b_plot)
ax.set_ylabel("Score")
ax.set_ylim([0, 1.12])
ax.set_title("Model Comparison — Cup Type Classification (4-class)\nGroupKFold (K=5) Mean +/- Std",
             fontweight="bold")
ax.legend(loc="upper right")
fig.tight_layout()
fig.savefig(OUT_FIGS / "metrics_comparison_cup_type.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  [OK] Saved -> outputs/figures/metrics_comparison_cup_type.png")

# ── 5g. Per-class F1 heatmap — Task B ────────────────────────────────────
f1_cols = [f"F1_{c}" for c in class_names]
heatmap_data = summary_b.set_index("Model")[[f"{c}_mean" for c in f1_cols]]
heatmap_data.columns = class_names
...
ax.set_title("Per-Class F1-Score Heatmap — Cup Type Classification\n(Mean across 5 folds)",
             fontweight="bold")
ax.set_xlabel("Cup Class")
ax.set_ylabel("Model")
fig.tight_layout()
fig.savefig(OUT_FIGS / "f1_heatmap_cup_type.png", dpi=300, bbox_inches="tight")
plt.close(fig)
print("  [OK] Saved -> outputs/figures/f1_heatmap_cup_type.png")
